...
export default function App() {
  ...
  const payoutUSD = async (amountInUSD) => {
    ...
    const result = await payableContract.interface.decodeFunctionResult("transfer(address,uint256)", data);
        ...
        <div className="h-full p-1 mx-auto bg-white">
          <div className="flex items-center justify-between h-10 p-2">
            ...
            <div className="space-x-2">
              <button onClick={ () => payoutUSD(parseInt(payoutInUSD)) } className="px-4 py-2 text-sm font-medium text-white bg-green-500 border border-transparent rounded-md cursor-pointer whitespace-nowrap hover:bg-opacity-75" >
                Approve
              </button>
